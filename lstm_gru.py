...
def visualizeAndSave(cryptocurrencies, filenametags, svgfilenametags):
    for c in cryptocurrencies:
        for i in range(1, len(filenametags)):
            ...
            elif "bayes" in filenametags[i]:
                plt.title(f'{c} záró árfolyam előrejelzés Bayes optimalizált SVM modellel')
            elif "evolution" in filenametags[i]:
                plt.title(f'{c} záró árfolyam előrejelzés Evolúciós algoritmussal optimalizált SVM modellel')
            else:

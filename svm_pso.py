...
for c in cryptos:
    ...
    for i in range(0, 30):
        if not firsttime:
            df = pd.read_csv(f'.\Exchange Rates\{c}-USD_p_svm_grid.csv', index_col=False)
            df = dataPreprocessingAndSvmTeaching(df, rbf)
        ...
        df.to_csv(f'.\Exchange Rates\{c}-USD_p_svm_grid.csv', index=False)
        firsttime = False

visualizeAndSave(cryptos, ['', '-USD_p_svm_grid.csv'], ['', '-USD_svm_grid.svg'])
...
#Avg oszlop hozzáadása
def addAvgColumn(dataframe): 
...
df = pd.read_csv('BTC-USD.csv', index_col='Date', parse_dates=['Date'])
addAvgColumn(df)

training_set = df[:'2021'].iloc[:,6:7].values
test_set = df['2021':].iloc[:,6:7].values

df["Avg"][:'2021'].plot(figsize=(16,4),legend=True)
df["Avg"]['2021':].plot(figsize=(16,4),legend=True)
plt.legend(['Training set (2021 előtti értékek)','Test set (2021 és utáni értékek)'])
plt.title('Bitcoin napi átlagára')
plt.show()
